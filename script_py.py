...
# CONFIGURATION POUR CONNEXION RÉSEAU LOCAL
DB_CONFIG = {
    "host": "192.168.1.102",  # Adresse IP de la base de données sur le réseau local
    "port": 5433,
...
# CONFIGURATION BASE DE DONNÉES LOCALE (pour lookup des véhicules)
LOCAL_DB_CONFIG = {
    "host": "localhost",
    "port": 5432,
    "database": "safetrack_db",
...
# Cache pour DevEUI -> Vehicle ID
vehicle_cache: Dict[str, Optional[int]] = {}

def get_vehicle_id_by_deveui(dev_eui: str) -> Optional[int]:
    """
    Recherche l'ID du véhicule à partir du DevEUI dans la base de données LOCALE
    Utilise un cache pour éviter les requêtes répétées
    """
    # Vérifier le cache d'abord
    if dev_eui in vehicle_cache:
        return vehicle_cache[dev_eui]
    
    try:
        # Connexion à la base de données LOCALE pour chercher le véhicule
        local_conn = psycopg2.connect(**LOCAL_DB_CONFIG, client_encoding='utf8')
        cur = local_conn.cursor(cursor_factory=RealDictCursor)
        # Recherche insensible à la casse (DevEUI peut être en majuscules ou minuscules)
        cur.execute("SELECT id_vehicule FROM vehicule WHERE LOWER(deveui) = LOWER(%s)", (dev_eui,))
        result = cur.fetchone()
        local_conn.close()
        
        if result:
            vehicle_id = result['id_vehicule']
            vehicle_cache[dev_eui] = vehicle_id
            print(f"✅ Véhicule trouvé: DevEUI {dev_eui} → ID {vehicle_id}")
            return vehicle_id
        else:
            # Mettre None en cache pour éviter de requêter à nouveau
            vehicle_cache[dev_eui] = None
            print(f"⚠️  DevEUI {dev_eui} n'est pas associé à un véhicule enregistré dans la BD locale")
            return None
    except Exception as e:
        print(f"❌ Erreur lors de la recherche du véhicule dans la BD locale: {e}")
        return None

def send_position_to_backend(data: dict, dev_eui: str, vehicle_id: int, payload: str) -> bool:
    """
    Envoie une position GPS au backend local via HTTP POST
    """
    try:
        ...
        position_data = {
            ...
            "timestamp_gps": data['horodatage'],
            "fix_status": data.get('fix_mode', 1),
            "satellites": data.get('satellites', 0) if data.get('satellites') != 'N/A' else None,
            "hdop": None,  # Non disponible dans les données actuelles
            "statut": "ACTIF",
            "dans_zone": None,  # Sera calculé par le backend/trigger
        ...
        if response.status_code == 200:
            print(f"✅ Position envoyée au backend (Vehicle ID: {vehicle_id})")
            return True
        else:
                ...
                for msg in messages:
                    ...
                    if msg['text_payload']:
                        ...
                        if data and data.get('valid'):
                            ...
                            vehicle_id = get_vehicle_id_by_deveui(msg['dev_eui'])
                            if vehicle_id:
                                send_position_to_backend(data, msg['dev_eui'], vehicle_id, payload)
                            else:
                                print(f"⏭️  Position ignorée (DevEUI non enregistré)")
